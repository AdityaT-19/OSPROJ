...
class MemoryBlock {
    ...
    void firstFit(int pn) {
        int ans = -1;
        for(int i = 0; i < divs; i++) {
            if(free[i] && processes[pn] <= memory[i]) {
                free[i]=false;
    ...
    void bestFit(int pn) {
        ...
        for(int i = 0; i < divs; i++) {
            if(free[i] && processes[pn] <= memory[i]) {
                if(memory[i] - processes[pn] < curr) {
                    curr = memory[i] - processes[pn];
                    ans = i;
                }
            }
        }
    ...
    void printTable(int pos) {
        ...
        int pnum=0;
        for (int i = 0; i < divs; i++) {
                System.out.print("|\t" + (i + 1) + " \t " +  memory[i] + "  \t\t " + ((!free[i]) ? "F \t\t" : "NF ") + " " + ((free[i])? "\t\t\t":("Process " + (++pnum))) + "\t\t\t"+(((frag[i])==0)? "":frag[i])+"\t\t|");
            System.out.println(' ');
        }
        System.out.print("+---------------------------------------------------------------------------------------------+\n");
    }
